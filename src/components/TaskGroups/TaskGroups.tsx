...
class TaskGroups extends React.Component<State> {
  ...
  handleItemClick = (id: number) => {
    ...
    tasks.forEach((task: ListItemObject) => task.completedAt && completedSet.add(task.id));
    
    const task = tasks.find(item => item.id === id);
    if (!task) return;

    const completedAt = (!task.completedAt && (!task.dependencyIds.length || task.dependencyIds.every((id: number) => {
  ...
  render() {
    ...
    const sortedTasks: TaskListMap = tasks.reduce((result: TaskListMap, item: ListItemObject) => {
      return {
        ...result,
        [item.group]: [
          ...(result[item.group] || []),
          ...
          <div className="col-6 mt-5">
            <div className="row mb-4 w-100">
              ...
              <div className="col-6 d-flex align-items-end justify-content-end">
                {selectedGroup && <a href="#" onClick={this.handleGroupClick} className="h4 text-primary mb-0">ALL GROUPS</a>}
              </div>
            </div>
            ...
            {!selectedGroup
              ? Object.keys(sortedTasks).map((group: string) => {
                  ...
                  const numberOfCompleted = items.filter((item: ListItemObject) => item.completedAt).length;
                  return (
                    <li key={group} id={group} onClick={this.handleGroupClick} style={{ borderRight: '0', borderLeft: '0' }} className="list-group-item py-4 d-flex flex-row align-items-center">
                      <div style={{ height: '32px', width: '32px' }} className="mr-5">
                        <img src={groupSVG} />
                      </div>
                      <div className="d-flex flex-column text-left">
                        <h6 className="font-weight-bold mb-0">{group}</h6>
                        <h6 className="text-muted mb-0">{numberOfCompleted} OF {items.length} TASKS COMPLETE</h6>
                      </div>
                    </li>
                  );
                })
              : <TaskList items={sortedTasks[selectedGroup]} lockedSet={lockedSet} onItemClick={this.handleItemClick}/>
            }
            </ul>
          </div>
          <div className="col-3" />
...
function recursiveReset(tasks: ListItemObject[], id: number, collectionToReset: Set<number>) {
  let subIds: number[] = [];
  tasks.forEach(task => {
