...
const ListItem = ({ id, group, status, task, onItemClick }: ListItemProps) => {
  return (
    <li
      key={group}
      onClick={() => onItemClick(id, group)}
      style={{ borderRight: "0", borderLeft: "0" }}
      className="list-group-item py-4 d-flex flex-row align-items-center"
    >
      <div
        style={{ height: "24px", width: "24px" }}
        className="mr-5 d-flex justify-center align-center"
      >
        <img src={statusToSVG[status]} className="h-100 w-100" />
      </div>
      <h6
        className={`font-weight-bold mb-0${
          status === ITEM_STATUS.LOCKED ? " text-muted" : ""
        }${status === ITEM_STATUS.COMPLETED ? " strike-through" : ""}`}
      >
        {task}
      </h6>
    </li>
